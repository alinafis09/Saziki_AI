...
// Instagram Downloader for Saziki Bot with Dynamic Loading Animation
...
const STAGES = [
    '[ {sym} ] Analyzing Instagram link...',
    '[ {sym} ] Fetching media data...',
    '[ {sym} ] Processing content...',
    '[ {sym} ] Preparing download...'
];
...
let handler = async (m, { conn, usedPrefix, command, text }) => {
    ...
    const waitMsg = await m.reply(STAGES[0].replace('{sym}', LOADING_SYMBOLS[0]));
    const progressKey = waitMsg.key;
    
    // Start animation loop - updates every 1 second
    const animationInterval = setInterval(async () => {
        symbolIndex = (symbolIndex + 1) % LOADING_SYMBOLS.length;
        
        // Advance stage every 4 symbols (every 4 seconds)
        if (symbolIndex === 0 && stage < STAGES.length - 1) {
            stage++;
        }
        
        const animatedText = STAGES[stage].replace('{sym}', LOADING_SYMBOLS[symbolIndex]);
    ...
    try {
        ...
        const metadata = result.metadata || { caption: '', username: '', like: 0, comment: 0, isVideo: false };

        const caption = `𝗦𝗮𝘇𝗶𝗸𝗶 𝗱𝗼𝘄𝗻𝗹𝗼𝗮𝗱𝗲𝗿 ⎚

𝗟𝗼𝗮𝗱𝗶𝗻𝗴 𝘀𝘂𝗰𝗰𝗲𝘀𝘀𝗳𝘂𝗹𝗹𝘆 ⎙`.trim();

        // Stop animation
        clearInterval(animationInterval);
        
        // Update to final status before sending
        await conn.sendMessage(m.chat, {
            text: `[ 📥 ] 𝗟𝗼𝗮𝗱𝗶𝗻𝗴... ${mediaUrls.length} file${mediaUrls.length > 1 ? 's' : ''}...`,
            edit: progressKey
        });
        ...
        // Keep error message for 3 seconds then delete
        setTimeout(async () => {
            await conn.sendMessage(m.chat, { delete: progressKey }).catch(() => {});
        }, 3000);
    }
