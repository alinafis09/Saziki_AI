...
const src = join(__dirname, '..', 'assets'); // تعديل المسار
...
const genSVG = async ({
    ...
    const el = {
        code: ['#_1661899539392 > g:nth-child(6) > image', imageSetter, await toBase64(await toImg(barcode(wid.replace(/[^0-9]/g, '')), 'png'), 'image/png')],
...
const render = async ({
    ...
    title = '',
    text = '',
    background = toBase64(readFileSync(join(src, 'images', 'backgrounds', 'bakground-1.jpeg')), 'image/jpeg'),
} = {}, format = 'png') => {
